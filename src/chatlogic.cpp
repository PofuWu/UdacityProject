...
ChatLogic::ChatLogic()
{
    ...
    // Task 5: move to LoadAnswerGraphFromFile function.
...
ChatLogic::~ChatLogic()
{
    ...
    // Task 3, _node will be automatically deleted via its smart pointer properties
    // delete all nodes
    ...
    // delete all edges
    //Task 4: No need because already create a unique ptr.
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //Task3: Change GraphNode pointer to unique
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            //Task3: Change GraphNode pointer to unique pointer
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); //unique ptr use get()
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // move the ownership to childNodes.
                        }
    ...
    //Task 5: remove old instance.
    //_chatBot->SetRootNode(rootNode);
    //rootNode->MoveChatbotHere(_chatBot);

    // Task5: create new Chatbot instance.
